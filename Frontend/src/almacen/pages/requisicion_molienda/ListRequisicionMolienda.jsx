...
import { verifyRequisicionMolienda } from "./../../helpers/requisicion-molienda/verifyRequicisionMolienda";
// IMPORTACIONES PARA EL FEEDBACK
...
export const ListRequisicionMolienda = () => {
  ...
  // ESTADOS PARA EL DIALOG DE VERIFICACION
  const [open, setOpen] = React.useState(false);
  const [itemVerify, setItemVerify] = useState({
    idItemVerify: 0,
  });
  const { idItemVerify } = itemVerify;
  ...
  // ************** VERIFICAR REQUISICION MOLIENDA **************

  const openDialogVerificarRequisicion = (id) => {
    setItemVerify({
      idItemVerify: id,
    });
    handleOn();
  ...
  const verificarRequisicionMolienda = async () => {
    const resultPeticion = await verifyRequisicionMolienda(idItemVerify);
    const { message_error, description_error } = resultPeticion;
    console.log(resultPeticion);
    if (message_error.length === 0) {
      // actualizamos nuestra data temporal y fisica
      const updateAfterVerifyData = dataRequisicion.map((element) => {
        if (element.id === idItemVerify) {
          return {
            ...element,
            idReqMolEst: 4,
            desReqMolEst: "Verificado",
          };
        } else {
          return element;
        }
      });
      setdataRequisicion(updateAfterVerifyData);
      setdataRequisicionTemp(updateAfterVerifyData);
      // cerramos el dialog
      handleClose();
      // mostramos el feedback
      setfeedbackMessages({
        style_message: "success",
        feedback_description_error: "Se verificó la requisicion",
      });
      handleClickFeeback();
    } else {
      setfeedbackMessages({
  ...
  return (
    <>
      <div className="container-fluid">
        ...
        {/* DIALOG DE VERIFICACION DE ENTREGA REQUISICION MOLIENDA */}
        <Dialog
          open={open}
          onClose={handleClose}
        ...
        >
          <DialogTitle className="fs-3 text-success" id="alert-dialog-title">
            Verificacion
          </DialogTitle>
          <DialogContent>
            <DialogContentText id="alert-dialog-description">
              ¿Deseas verificar la entrega de la requisicion de molienda?
            </DialogContentText>
          </DialogContent>
          <DialogActions>
            <Button
              variant="contained"
              color="error"
              onClick={verificarRequisicionMolienda}
            >
              Aceptar
            </Button>
